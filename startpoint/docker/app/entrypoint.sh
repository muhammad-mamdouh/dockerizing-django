...
# Check who owns the working directory - %u > retreives user ID of owner
...
# Test to see if the user exists in the docker container
PYTHON_RUN_USER_TEST=$(grep "[a-zA-Z0-9\-\_]*:[a-zA-Z]:${PYTHON_RUN_UID}:" /etc/passwd)

# Update the container user to the configured UID and group
